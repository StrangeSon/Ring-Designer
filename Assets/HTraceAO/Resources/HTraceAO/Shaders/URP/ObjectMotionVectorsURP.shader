Shader "Hidden/HTraceAO/ObjectMotionVectorsURP"
{
    SubShader
    {
        Pass
    ...
    #pragma multi_compile _ DOTS_INSTANCING_ON
        ...
        #pragma target 4.5 DOTS_INSTANCING_ON
            ...
            Varyings vert(Attributes input)
            {
                ...
                const float4 posWS = mul(UNITY_MATRIX_M, input.position);
                const float4 prevPosWS = mul(UNITY_PREV_MATRIX_M, prevPos);

                output.positionCSNoJitter = mul(H_MATRIX_VP, posWS);
                output.previousPositionCSNoJitter = mul(H_MATRIX_PREV_VP, prevPosWS);
                output.MotionMask = length(posWS - prevPosWS) > 0.0001 ? 1 : 0;
